...
import com.training.hospital.dto.param.PowerAccountRegisterParam;
import com.training.hospital.dto.param.PowerAccountStatusParam;
import com.training.hospital.dto.param.PowerAccountUpdatePasswordParam;
...
import com.training.hospital.dto.param.PowerAccountUpdateOpenidParam;
...
public class PowerAccountController {
    ...
    @ApiOperation(value = "openid账号登录", notes = "传入 账号名称")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "name", value = "账号名称", paramType = "query", dataType = "String",
                    required = true),
    ...
    public CommonResult<String> loginAccountByopenid(@RequestParam String name) {
        if (!accountService.count(name)) {
            return CommonResult.validateFailed("不存在，该账号名称！");
        }

        Optional<String> stringOptional = accountService.loginByopenid(name);
    ...
    @ApiOperation(value = "根据openid获取用户信息", notes = "传入 openid 返回 账号 密码")

    @RequestMapping(value = "/openid", method = RequestMethod.POST)
    public CommonResult<Map<String, Object>> GetOpenidAccount(@RequestParam String openid) {
        Optional<PowerAccount> accountOptional = accountService.getByopenid(openid);
        ...
        if (accountOptional.isPresent()) {
            ...
            info.put("password", account.getPassword());
            // TODO 这里需要返回用户相应的角色
    ...
    @ApiOperation(value = "更新账号openid", notes = "传入 账号编号 openid")
    @RequestMapping(value = "/openidmodify", method = RequestMethod.GET)
    public CommonResult updateAccountOpenid(@RequestParam String username, @RequestParam String openid) {
    ...
    @ApiOperation(value = "按名获取当前账号信息", notes = "传入 名字")
    @RequestMapping(value = "/infoByname", method = RequestMethod.GET)
    public CommonResult<Map<String, Object>> getCurrentAccountByname(@RequestParam String username) {
